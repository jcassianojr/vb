...
' -------------------------------------------------------------------------
' 8. FUNÇÕES DE SEGURANÇA E EXTENSÕES
' -------------------------------------------------------------------------
Public Function IsQuerySafe(ByVal cSQL As String) As Boolean
    Dim cCleanSQL As String, cNewSQL As String, i As Long, charCode As Long
    Dim aForbiddenWords As Variant
    aForbiddenWords = Array("DELETE", "DROP", "UPDATE", "INSERT", "CREATE", "ALTER", "TRUNCATE", "EXEC", "EXECUTE", "GRANT", "REVOKE", "INTO")
    
    cCleanSQL = UCase(cSQL)
    cNewSQL = ""
    For i = 1 To Len(cCleanSQL)
        charCode = Asc(Mid(cCleanSQL, i, 1))
        If (charCode >= 65 And charCode <= 90) Or (charCode >= 48 And charCode <= 57) Then
            cNewSQL = cNewSQL & Mid(cCleanSQL, i, 1)
        Else
            cNewSQL = cNewSQL & " "
        End If
    Next i
    
    cCleanSQL = Trim(cNewSQL)
    Do While InStr(cCleanSQL, "  ") > 0
        cCleanSQL = Replace(cCleanSQL, "  ", " ")
    Loop
    
    Dim aTokens() As String
    aTokens = Split(cCleanSQL, " ")
    If UBound(aTokens) < 0 Or aTokens(0) <> "SELECT" Then
        IsQuerySafe = False
        Exit Function
    End If
    
    For i = LBound(aForbiddenWords) To UBound(aForbiddenWords)
        If InStr(cCleanSQL, aForbiddenWords(i)) > 0 Then
            IsQuerySafe = False
            Exit Function
        End If
    Next i
...
Public Function IsQueryDestructive(ByVal cSQL As String) As Boolean
    Dim cCleanSQL As String, cNewSQL As String, i As Long, charCode As Long
    Dim aDestrutivos As Variant
    aDestrutivos = Array("DROP", "ALTER", "CREATE", "TRUNCATE", "GRANT", "REVOKE")
    ...
    For i = LBound(aDestrutivos) To UBound(aDestrutivos)
        If InStr(cCleanSQL, aDestrutivos(i)) > 0 Then
            IsQueryDestructive = True
            Exit Function
        End If
    Next i
    IsQueryDestructive = False
End Function

Public Function IsSQLiteExt(ByVal cFileName As String) As Boolean
    If Trim(cFileName) = "" Then
        IsSQLiteExt = False
        Exit Function
    End If
    Dim cExt As String
    cExt = LCase(Mid(cFileName, InStrRev(cFileName, ".")))
    IsSQLiteExt = (cExt = ".sqlite" Or cExt = ".sqlite3" Or cExt = ".db3" Or cExt = ".fossil")
End Function

Public Function IsFirebirdExt(ByVal cFileName As String) As Boolean
    If Trim(cFileName) = "" Then
        IsFirebirdExt = False
        Exit Function
    End If
    Dim cExt As String
    cExt = LCase(Mid(cFileName, InStrRev(cFileName, ".")))
    IsFirebirdExt = (cExt = ".fdb" Or cExt = ".gdb" Or cExt = ".ib")
End Function
